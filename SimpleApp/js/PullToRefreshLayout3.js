...
export default class PullToRefreshLayout3 extends Component {
    ...
    _updateNativeStyles() {
        this.content && this.content.setNativeProps(this.contentStyles);
    }

    // 加载完成，从基准点回到0；或者从小于基准高度的距离回到0
    resetHeader() {
        console.log(TAG, 'resetHeader');
        const { backTime } = this.props;
        this.contentStyles.style.marginTop = 0;
        this._updateNativeStyles();
    ...
    refreshStateHeader() {
        ...
        this.contentStyles.style.marginTop = headHeight;
        this._updateNativeStyles();
        LayoutAnimation.configureNext({
